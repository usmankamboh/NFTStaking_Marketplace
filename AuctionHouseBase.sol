...
abstract contract AuctionHouseBase is OwnableUpgradeable,  ReentrancyGuardUpgradeable, RaribleTransferManager, TransferExecutor {
    ...
    //  default minimal auction duration and also the time for that auction is extended when it's about to end (endTime - now < EXTENSION_DURATION)
    ...
    //  maximum fee base point
    uint internal constant MAX_FEE_BASE_POINT = 1000;

    //  mapping to store eth amount that is ready to be withdrawn (used for faulty eth-bids)
    ...
    //  minimal bid increase in base points
    ...
    //  increments auctionId and returns new value
    ...
    //  returns true if newAmount is enough for buyOut
    ...
    //  check that msg.value more than bid amount with fees and return change
    ...
    //  returns true if auction in progress, false otherwise
    ...
    //  reserves new bid and returns the last one if it exists
    function reserveBid(
        address buyAsset,
        address oldBuyer,
        address newBuyer,
